...
const escapeForLike = (value) => value.replace(/[\\%_]/g, "\\$&");
...
const buildSnippet = (text, keyword) => {
...
export const upsertCapture = (db, capture) => {
  ...
  `);
    ...
    const persisted = db
        ...
        .get({ page_id: capture.pageId });
    return {
        id: persisted?.id ?? capture.id,
        changes: result.changes
    };
};
export const searchCaptures = (db, keyword, limit, since) => {
...
export const getCaptureById = (db, id) => {
    const row = db
        .prepare(`
      SELECT
      id,
